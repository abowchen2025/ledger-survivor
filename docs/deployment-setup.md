...
- [ ] `backend` 服務 → 「Variables」分頁 → 「Raw Editor」，貼入（Railway 的 `${{Postgres.XXX}}` 是「參照變數」，會自動代入同專案 Postgres 服務的值；若你的 Postgres 服務名稱不是 `Postgres`，把大括號裡的名字改成實際名稱）：
  ...
  TZ=Asia/Taipei
  DEBUG=false
  API_KEY=<自己產生的隨機值，例如 openssl rand -hex 32>
  CORS_ALLOWED_ORIGINS=https://abowchen2025.github.io
  ```
...
| `railway up`：`Uploading...` 之後 `error sending request for url … Caused by: operation timed out`（2026-09-23 Deploy backend #9） | Railway 端（backboard.railway.com）網路瞬斷，與程式無關 | `.github/scripts/railway-up-retry.sh` 會對「上傳被接受前的網路錯誤」自動重試最多 3 次（間隔 10、30 秒）；三次都失敗就在 Actions 頁 Re-run failed jobs。建置／部署失敗（log 已出現 `Deployment:`）不會重試 |
